...
const path = require("path");// way to work within directory
const app = express();//stored express function
const hbs = require("hbs");//defined js template engine
...
const port = process.env.PORT || 3000;//defined port to render project
// console.log(path.join(__dirname, "../public"));
//stored  path in variable
const static_path = path.join(__dirname, "../public");
const template_path = path.join(__dirname, "../templates/views");
const partials_path = path.join(__dirname, "../templates/partials");
//use and set...one of the first thing to run
app.use(express.json());//parsed data
app.use(express.urlencoded({extended:false}));//parsing data
app.use(express.static(static_path));//accessing  css
app.set("view engine", "hbs");//hbs for templating 
app.set("views", template_path);// setting path to access views directory from any directory
hbs.registerPartials(partials_path);//reusing templates 
//handling requests and response to target location //rendering data 
app.get("/", (req, res) => {
    res.render("index");
    });
